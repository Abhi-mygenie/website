...
| **CR-33** | `fbc` (Meta Click Cookie) Forwarding to Browser Pixel + Server CAPI | **G3 ✅ Code shipped → G4** | **CODE LIVE (`gtm.js` patched 2026-06-27)** — owner GTM wireup pending in BOTH containers | 🟡 Owner GTM action | 1-line code change LIVE: `fbc: attr.fbc \|\| null` added to `buildLeadPayload()`. Cookie already captured in `attribution.js:148` (CR-2). Owner action: see `CR-33_Owner_GTM_Action_Brief.md` (10-min checklist) → create DLV + add to 3 GA4 tags (web) + create Event Data var + add to CAPI userDataList (server). Specs: `CR-33_FBC_Click_Cookie_Forwarding.md`. Expected EMQ lift +0.5–0.7 within 48hrs post-publish. |
| **CR-34** | Historical Lead Backfill — Extend source sync to pull all attributed contacts from July 1 2025 onwards | **G3 ✅ Implemented** | **DONE — 1,613 backfilled_leads with 919 attributed. Source sync date changed, 6-hour scheduler updated.** | 🟢 No | Changed `SOURCE_SYNC_AFTER_DATE` to `2025-07-01`. Added `SOURCE_SYNC_SCHEDULED_AFTER_DATE=2026-06-20` for 6-hr job. Merged mygenie_db→test_database. Files: `crm_sync.py`, `server.py`, `backend/.env`. |
| **CR-35** | Leads table to include `backfilled_leads` + direct lead attribution gap | **G1 — Registered** | **DISCOVERY** | 🟢 No | TWO parts: (A) `leads.py` `query_leads()` only reads `demo_requests`/`quotes`/`contact_messages` — excludes 1,613 `backfilled_leads`. Lost leads (Gffh, Himanshu, Prajjwal) invisible in leads table. Needs `_normalise_backfilled`. (B) Direct/organic website leads have `first_source=null` in Freshsales. Fix: backend to pass `first_source="website"` as default when no UTM source is present. This makes direct leads recoverable by source sync and visible in source-filtered views. |
| **CR-36** | Meta spend date filtering — funnel shows lifetime instead of selected period | **G1 — Registered** | **DISCOVERY** | 🟢 No | `_get_spend_by_source()` in `funnel.py` sums ALL `ad_spend` docs regardless of dashboard date filter. Meta API sync stores data with `date_start: 2025-01-01 → 2025-12-31` (lifetime). ₹11,07,220 shown for Jun 21-30 is actually 18-month total. CPL/CP Sched/CP Demo/CP Win all incorrect. |
| **CR-37** | Google Ads spend missing date ranges | **G1 — Registered** | **DISCOVERY** | 🟢 No | Google's 9 `ad_spend` docs have `date_start: None, date_stop: None`. Google sync not populating date fields. Tied to CR-36 — both need date-filtered spend queries. |
| **CR-38** | Move Ad Spend Upload widget to Ads Intelligence screen | **G1 — Registered** | **DISCOVERY** | 🟢 No | CSV upload widget on Leads page but unrelated to leads data. With live Meta+Google API, it's a backup that belongs on Ads Intelligence. UX cleanup, low priority. |
...
## CR-34 — Historical Lead Backfill (G1 Discovery + G2 Planning)

**Opened:** 2026-06-30
**Goal:** Extend the source sync to pull ALL Freshsales contacts with `first_source` set, created from **July 1, 2025 onwards** — not just after June 20, 2026. Ensures the leads dashboard and funnel analytics have complete historical data for all attributed leads.

---

### G1 Discovery — Current State

**Two sync mechanisms exist today:**

| Sync | What it pulls | Date range | Triggered by | Result |
|---|---|---|---|---|
| **Stage sync** (`run_sync` / `_run`) | Contacts at stages 3-6 (demo_scheduled/given/won/lost) | No date filter | Scheduled every 6 hours | 535 contacts in `backfilled_leads` |
| **Source sync** (`run_source_sync`) | Contacts with `first_source` NOT null | **After June 20, 2026 only** (`SOURCE_SYNC_AFTER_DATE`) | Manual trigger (`/api/cms/sync/trigger`) + paired with stage sync on trigger | 109 contacts in `backfilled_leads` |

**Also:** `run_source_backfill` (one-time endpoint `/api/cms/sync/source-backfill`) uses `BACKFILL_DATE_FROM = "2025-07-01"` but only pulls contacts matching `_SOURCE_BUCKETS` (google/facebook/website/chat source values). This ran once and contributed to the 644 total.

**Gap:** Contacts with `first_source` set, created between July 1 2025 and June 20 2026, that are NOT at stages 3-6 = **~853 contacts missing from MongoDB**.

### G1 Discovery — Impact Analysis

**Freshsales data (verified via API):**

| Query | Count |
|---|---|
| ALL contacts with `first_source` after July 1 2025 | **1,497** |
| Currently synced (after June 20 2026) | 169 |
| Already in `backfilled_leads` | 644 |
| **New contacts to sync (gap)** | **~853** |

**Breakdown by source bucket (after July 1 2025):**
- Facebook: 779
- Google: 207
- Website: 6
- Chat: 2

**Storage impact:** ~853 docs × ~500 bytes = **~0.4 MB** (safe — current usage ~488/512 MB)
**Sync time:** ~853 individual GETs × 2s rate limit = **~28 minutes** one-time

---

### G2 Planning — Approach

**Change:** Update `SOURCE_SYNC_AFTER_DATE` default from `"2026-06-20"` to `"2025-07-01"` in `crm_sync.py` line 485.

**How it works (no logic change, only date change):**

1. `run_source_sync()` discovers contacts with `first_source` NOT null, created after `SOURCE_SYNC_AFTER_DATE`
2. Compares against existing `backfilled_leads` with `first_source` set → identifies gap contacts
3. For each gap contact, does individual GET to fetch full data (cf_ fields, native attribution)
4. Upserts into `backfilled_leads` with:
   - `crm_status` = their current Freshsales stage (new/demo_scheduled/given/won/lost)
   - Full attribution: first_source, first_medium, first_campaign, latest_*, keyword, ad_set, gclid, fbclid, ad_id, adset_id, placement, utm_id, site_source_name, event_id
   - `$ifNull` preserves existing non-null values (no data loss on re-sync)
5. Existing contacts are not re-fetched (dedup via `existing_with_source` set)

**Files changed:** 1 file — `backend/crm_sync.py` (line 485, env default value)

**Env override available:** `SOURCE_SYNC_AFTER_DATE` env var — if set in `backend/.env`, overrides the code default. No code change needed if you prefer env-only.

**One-time run:** After the change, trigger `/api/cms/sync/source-sync` to pull the historical gap. Subsequent 6-hour scheduled syncs will only pick up new contacts (gap will be 0 after initial run).

**Risk:** LOW. No logic change. Same code path that already runs for 109 contacts — just wider date range. $ifNull prevents overwriting existing attribution. Upsert prevents duplicates.

**No frontend change.** The leads dashboard and funnel already read from `backfilled_leads` — 853 new docs will appear automatically.

---

### Gate Progress

- [x] **G1 Discovery** — DONE (2026-06-30). Current state mapped, gap quantified, impact assessed.
- [x] **G2 Planning** — DONE (2026-06-30). 1-line change identified, no blockers.
- [ ] **G3 Implementation**
- [ ] **G4 QA**
- [ ] **G5 Owner Smoke Test**
...
### Change Log
- 2026-06-30 — CR-34 REGISTERED (G1+G2 done). Root cause: `SOURCE_SYNC_AFTER_DATE` was `2026-06-20`, missing 853 attributed contacts from July 2025–June 2026. Also discovered and fixed: MongoDB Atlas 512MB quota full (writes blocked since June 29 — 3 leads lost to Freshsales-only). Dropped stale `pos_request_logs` (26.5MB) to unblock writes. Meta events issue separately identified: GTM CAPI tag has `TEST13788` test ID sandboxing all events — owner to remove from GTM.
