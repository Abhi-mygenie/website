...
| **CR-220** | **Enhanced Conversions broken on server-side Google Ads path (Path B).** `buildLeadPayload()` in `src/lib/gtm.js` sends `email`, `phone`, `first_name`, `last_name` as flat root-level parameters on every `dataLayer` push. Google Enhanced Conversions (server-side, via Stape-hosted GTM-KN4B3Q2H sgtmadsct tag) requires these fields inside a dedicated `user_data: { email_address, phone_number, address: { first_name, last_name } }` object. The `user_data` key is confirmed **completely absent** from the entire codebase. GA4 event arriving at server GTM therefore carries no user_data → server-side GAds tag (sgtmadsct) has no Enhanced Conversions data to forward → Google Ads diagnostic: "Enhanced Conversions: Setup issues detected. Managed through API." Path A (client-side GAds tag) works because it maps flat fields directly on the tag itself. Affects all 5 form entry points. Source: Technical Brief Sep 5 2026 + code investigation. | 🔲 Open — **Two complementary fixes: (A) GTM-only: create "User-Provided Data" variable in web GTM, map flat `email`→`email_address`, `phone`→`phone_number`, `first_name`/`last_name`→`address.*`, attach to "GA4 - Book demo" tag's User Data section. No code change. (B) Code-side: add `user_data: { email_address, phone_number, address: { first_name, last_name } }` block to `buildLeadPayload()` return object alongside existing flat keys. 1 function, propagates to all 5 entry points. All source values already normalized (normEmail/normPhone). Recommended: both A+B. Plan at `/app/memory/CR-220_Enhanced_Conversions_Impact_Analysis.md`.** | P0 | `src/lib/gtm.js:buildLeadPayload()` (Fix B) · web GTM container GTM-K5D84Z3L (Fix A) |
