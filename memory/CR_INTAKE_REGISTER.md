...
**Updated:** 2026-09-05 — CR-218 ✅ Done. CR-219 ✅ Done. CR-220 Fix B ✅ Done (user_data added to buildLeadPayload), Fix A + Fix C 🔲 pending GTM owner action.
...
| **CR-220** | **Enhanced Conversions broken — three stacked issues.** (1) EC mode set to "Automatic" on client-side GAds tag: Google scans DOM for `<input>` fields at conversion time, but ALL form entry points use a React stage machine (form→otp→calendly). `book_demo` fires during OTP stage's `onVerified` callback — form inputs are already unmounted. Google finds nothing → last successful EC ping Sep 2, 2026. Confirmed by Google Ads console screenshot ("Needs attention / Implement in-page code in addition to Automatic", Last ping date: Sep 2). (2) `user_data` container completely absent from `buildLeadPayload()` dataLayer push — `email`, `phone`, `first_name`, `last_name` sent flat at root level. Google's server-side GA4 → sGTM path requires dedicated `user_data: { email_address, phone_number, address: { first_name, last_name } }` object. `user_data`, `email_address`, `phone_number` — zero occurrences in codebase. (3) "GA4 - Book demo" GTM tag has no User-Provided Data variable → GA4 event to server container (GTM-KN4B3Q2H, Stape) has no user_data → server sgtmadsct tag has no EC data to forward. Source: Tech Brief Sep 5 2026 + code investigation + Google Ads console screenshot. | ✅ Fix B Done 2026-09-05 — `user_data: { email_address, phone_number, address: { first_name, last_name } }` added to `buildLeadPayload()`. All flat keys unchanged. Beta `main.5ded3b5e` live. Prod zip `main.aad795d7` at `/app/mygenie-prod-build.zip`. 🔲 Fix A + Fix C pending — **👤 GTM owner: (A) Create "User-Provided Data" variable `dlv - user_data` reading dataLayer `user_data`; attach to "GA4 - Book demo" tag User Data section. (C) On "GAds - Book Demo" tag: switch EC mode Automatic → Code; set same variable. GTM Preview test → Publish. Expected: "Last ping date" updates same day; "Needs attention" warning clears 24-72h.** | P0 | `src/lib/gtm.js:buildLeadPayload()` ✅ · web GTM GA4 tag Fix A 🔲 · web GTM GAds tag Fix C 🔲 |
