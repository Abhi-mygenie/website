...
| **CR-213** | All 11 solution page title tags use plural `s.name` ("Restaurants", "Cafés") while H1 uses singular ("Restaurant POS system", "Café POS system"). Mismatch across all 11 pages dilutes keyword consistency between title and H1. | 🔲 Open — **Option A: add `nameSingular` field to each sector in `sectors.js` (11 lines) + use `s.nameSingular \|\| s.name` in `SectorPage.jsx:62` (1 line). Title changes, H1 untouched, nav untouched. Zero risk.** | P2 | `src/data/sectors.js` (11 edits) + `src/pages/SectorPage.jsx` (1 edit) |
| **CR-214** | Sitemap `lastmod` dates stale — sitemap is 100% complete (59 URLs, no missing/extra pages, `/demo` correctly excluded as noindex). But CR-187/189 (H1 + solutions desc updates, 2026-09-04) and CR-208 (2026-09-05) changed page content after the last sitemap update (2026-08-26). 11 sector pages + homepage need updated dates. | 🔲 Open — **12 `lastmod` date edits in `public/sitemap.xml`. Zero functional change. Can combine with CR-210/211/213 in one build.** | P2 | `public/sitemap.xml` |
