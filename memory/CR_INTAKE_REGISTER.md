...
| **CR-218** | `QAPage` schema **invalid** on all 20 pages — `answerCount` missing from every `Question` object (Google requires it). 60 Question objects across 5 files missing this field. Google marks schema invalid → **zero Q&A rich results eligible on homepage, 11 sector pages, AI page, resources, 6 product pages**. Confirmed: `answerCount present: False` in prerendered build. | 🔲 Open — **11 edits across 5 files: `seo.js` (7 blocks) + `SectorPage.jsx` + `AiPage.jsx` + `ProductPage.jsx` + `Resources.jsx` (1 inline map each). Add `answerCount: 1` per Question, `upvoteCount: 0` per Answer. Zero visual change, rebuild required.** | P0 | `src/lib/seo.js` + 4 page files |
