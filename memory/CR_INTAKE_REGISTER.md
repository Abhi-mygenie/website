...
| **CR-267** | `backend/.env` and `frontend/.env` are untracked but **not in `.gitignore`** — one `git add .` / "Save to GitHub" would publish live tokens (Calendly, SMS, CMS JWT, Mongo). Fix = 2 lines in `.gitignore`. | 🔲 **INTAKE — owner chose "register only, do later"** | P1 | INTAKE | `governance/BASELINE_2026-09-13.md` §B-9 |
