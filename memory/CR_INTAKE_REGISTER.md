...
**Updated:** 2026-09-05 — CR-218 ✅ Done (QAPage answerCount). CR-219 ✅ Done (GA4 Consent Mode EEA region fix). CR-220 registered (Enhanced Conversions server-side gap, P0).
...
| **CR-210** | `/solutions/ice-cream-desserts` H1 missing "POS system & billing software" — CR-187 fixed 8 sectors but missed the 11th. Keywords in title tag but absent from page body — hurts Google Ads Quality Score for ice cream ad group. | 🔲 Open — **1 search_replace: `sectors.js:299`. Change `"Ice cream shop POS —"` → `"Ice cream shop POS system & billing software —"`. Zero risk, 1 rebuild.** | P1 | `src/data/sectors.js` line 299 |
| **CR-211** | No preconnect hints for FB Pixel (`connect.facebook.net`) and Cloudflare Insights (`static.cloudflareinsights.com`) — both injected by GTM tags, not in source. ~250ms connection latency per domain (~500ms total) after GTM fires. GTM preconnect done (CR-209), these two remain. | 🔲 Open — **2 `<link rel="preconnect">` tags in `public/index.html` after CR-209 block. Zero risk. Can combine with CR-210 in one rebuild.** | P1 | `public/index.html` |
| **CR-212** | `/solutions/bars-and-pubs` and `/solutions/hotels` return HTTP 404 — redirect rules exist in `redirects.js` + `_redirects` but `nginx-redirects.conf` was never created, and `static-server.js` has no redirect logic. Googlebot sees 404. Client-side `<Navigate>` only works for JS-enabled browsers. | 🔲 Open — **Part A (code): add both URLs to `prerender.js` extraRoutes → prerendered folder found → static-server returns 200. Part B (owner): add `return 301` directives to production nginx config.** | P1 | `scripts/prerender.js` (Part A) · nginx config (Part B — owner) |
...
| **CR-215** | TrustBand marquee: 56 logos duplicated to 112 DOM nodes (`const loop = [...items,...items]`) → single flex container with 112 children → Style & Layout 431ms, Rendering 335ms, DOM 1,089. Identified in Lighthouse audit 2026-09-05. | ✅ Done 2026-09-05 Part A — added `style={{willChange:"transform",contain:"layout style"}}` to marquee div. Build hash 7d5ff572. Est +2-3 pts. Part B (owner): reduce TRUST_LOGOS 56→~20 logos still open. | P2 | `src/components/home/TrustBand.jsx` (Part A done) · `src/data/content.js` (Part B) |
...
| **CR-217** | Production nginx serves all `/brand/` assets with only **4h cache TTL** (`max-age=14400`). Lighthouse shows 40 resources / 466 KiB re-downloaded on every return visit after 4h. Pod's `static-server.js` correctly serves 7d for `/brand/`, 1yr for `/static/` — production nginx is overriding these. | 🔲 Open — **👤 Owner: add nginx cache directives: `/static/*` → 1yr immutable, `/brand/*` → 7d, `/fonts/*` → 1yr. Alternative: CF-5 Cloudflare Cache Rule (same result, no nginx access needed).** | P1 | Production nginx config · Cloudflare CF-5 |
...
| **CR-220** | **Enhanced Conversions broken — three stacked issues.** (1) EC mode set to "Automatic" on client-side GAds tag: Google scans DOM for `<input>` fields at conversion time, but ALL form entry points use a React stage machine (form→otp→calendly). `book_demo` fires during OTP stage's `onVerified` callback — form inputs are already unmounted. Google finds nothing → last successful EC ping Sep 2, 2026. Confirmed by Google Ads console screenshot ("Needs attention / Implement in-page code in addition to Automatic", Last ping date: Sep 2). (2) `user_data` container completely absent from `buildLeadPayload()` dataLayer push — `email`, `phone`, `first_name`, `last_name` sent flat at root level. Google's server-side GA4 → sGTM path requires dedicated `user_data: { email_address, phone_number, address: { first_name, last_name } }` object. `user_data`, `email_address`, `phone_number` — zero occurrences in codebase. (3) "GA4 - Book demo" GTM tag has no User-Provided Data variable → GA4 event to server container (GTM-KN4B3Q2H, Stape) has no user_data → server sgtmadsct tag has no EC data to forward. Source: Tech Brief Sep 5 2026 + code investigation + Google Ads console screenshot. | 🔲 Open — **Three-part fix: (Fix B — code) Add `user_data: { email_address: email, phone_number: phone, address: { first_name, last_name } }` block to `buildLeadPayload()` in `src/lib/gtm.js`. 1 function, propagates to all 5 entry points automatically. (Fix A — GTM) Create "User-Provided Data" variable in web GTM reading dataLayer `user_data`; attach to "GA4 - Book demo" tag User Data section. (Fix C — GTM) On "GAds - Book Demo" tag: switch Enhanced Conversions mode from "Automatic" to "Code"; set same User-Provided Data variable. Recommended order: Fix B → rebuild/deploy → Fix A+C in GTM → verify in GTM Preview → Publish. Full plan at `/app/memory/CR-220_Enhanced_Conversions_Impact_Analysis.md`.** | P0 | `src/lib/gtm.js:buildLeadPayload()` (Fix B) · web GTM GTM-K5D84Z3L GA4 tag (Fix A) · web GTM GAds tag (Fix C) |
