...
| **CR-215** | TrustBand marquee: 56 logos duplicated to 112 DOM nodes (`const loop = [...items,...items]`) → single flex container with 112 children → Style & Layout 431ms, Rendering 335ms, DOM 1,089. Identified in Lighthouse audit 2026-09-05. | 🔲 Open — **Part A (code, now): add `style={{willChange:"transform",contain:"layout style"}}` to marquee div — 1 line, removes GPU recompositing cost. Est +2-3 pts. Part B (owner): reduce TRUST_LOGOS 56→~20 logos → 40 DOM nodes (−64%).** | P2 | `src/components/home/TrustBand.jsx` (Part A) · `src/data/content.js` (Part B) |
