...
**Updated:** 2026-09-05 — CR-218 ✅ Done (QAPage answerCount fix, builds `main.69e93aec`/`main.4abff48f`). CR-219 registered (GA4 Consent Mode region gap, P0).
...
| **CR-219** | **GA4 blocked by global `analytics_storage: denied` default — no EEA region restriction.** `gtag('consent','default',{analytics_storage:'denied',...})` in `public/index.html` applies to ALL visitors worldwide (no `region` parameter). For MyGenie's primary Indian audience (GDPR-exempt), most users scroll past the consent banner without clicking Accept — `mg_consent` key never set in localStorage → `setDefaultConsent()` in `gtm.js:100-101` finds `null` → no `updateConsent()` call → consent stays `denied` for entire session → **GA4 does not fire**. Meta Pixel is unaffected because it does not check `analytics_storage`. This is the confirmed root cause of GA4 under-reporting vs Meta. Investigated 2026-09-05, full analysis at `HANDOVER_2026-09-05_Session7.md`. | 🔲 Open — **2 edits in `public/index.html` + `src/lib/gtm.js:setDefaultConsent()`: (1) Scope `denied` defaults to EEA/GB only via `region:['EEA','GB']` parameter; (2) Add second `gtag('consent','default',{analytics_storage:'granted',...})` call (no region = everyone else). Zero visual change, one rebuild required.** | P0 | `public/index.html` · `src/lib/gtm.js` |
