  ...
  try {
    for (const route of ROUTES) {
      ...
      await page.evaluate(() => {
        ...
        // CR-238: preload the route's JS chunks (recorded by index.js before render) so they
        // download in parallel with main.js. CmsAdminLayer / below-fold chunks load post-render → excluded.
        document.querySelectorAll('head link[rel="preload"][as="script"]').forEach((l) => l.remove());
        (window.__pageChunks || []).forEach((href) => {
