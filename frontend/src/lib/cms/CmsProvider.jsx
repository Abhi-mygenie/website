...
export function CmsProvider({ children }) {
  // Safe static defaults — match what puppeteer rendered at prerender time.
  // localStorage / window reads moved to useEffect below (hydration-safe).
  ...
  // Restore admin state from localStorage after hydration — not during render.
  // Keeps initial render identical to prerendered HTML (no hydration mismatch).
  useEffect(() => {
    const t  = localStorage.getItem(TOKEN_KEY) || null;
    const e  = localStorage.getItem(EDITMODE_KEY) === "true";
    const p  = localStorage.getItem(PREVIEW_KEY) === "true";
    const q  = new URLSearchParams(window.location.search);
    const lo = q.get("admin") === "1" || window.location.hash === "#admin";
    if (t)  setToken(t);
    if (e)  setEditMode(true);
    if (p)  setPreview(true);
    if (lo) setLoginOpen(true);
  }, []);

  // Public CMS content — only fetched on blog pages or when admin is logged in.
  // All other pages use prerendered HTML which already has CMS content baked in.
  // This eliminates a 31-component re-render on every non-blog page visit (TBT fix).
  useEffect(() => {
    const isBlog   = window.location.pathname.startsWith("/blog");
    const hasToken = !!localStorage.getItem(TOKEN_KEY);
    if (!isBlog && !hasToken) {
      setPublishedLoaded(true); // unblocks BlogPost.jsx loading gate for prerendered posts
      return;
    }
