    ...
    <footer className="bg-brand-deep border-t border-[#1e4a2e]" data-testid="pos-lp-footer">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex flex-col sm:flex-row items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center justify-center gap-4 text-xs text-[#5B7A68]">
          <span className="text-brand-muted" data-testid="pos-lp-footer-phone">{COMPANY.phone}</span>
          ...
          {PLANS.map((plan, i) => (
            <Reveal key={plan.name} delay={i * 0.08}>
              <div className={`border rounded-3xl p-8 relative ${plan.pop ? "border-brand-green bg-white shadow-[0_0_0_3px_rgba(16,185,129,0.12)]" : "border-brand-line bg-white"}`} data-testid={`pos-plan-${plan.name.toLowerCase()}`}>
                ...
                <button type="button" onClick={openSheet} className={`block w-full text-center py-3 rounded-xl text-sm font-bold transition-all ${plan.pop ? "bg-brand-green hover:bg-brand-greenDark text-white" : "border border-brand-green text-brand-green hover:bg-brand-green/10"}`} data-testid={`pos-plan-cta-${plan.name.toLowerCase()}`}>
                  Get Started
                </button>
              </div>
            </Reveal>
  ...
  return (
    <div className="bg-white" data-testid="restaurant-pos-system-page">
      ...
      <main className="pt-[72px]">
        {/* ── Hero ── */}
        <section className="bg-brand-sand pt-20 pb-16 lg:pt-40 relative overflow-hidden" data-testid="pos-lp-hero">
            ...
            <div className="grid lg:grid-cols-2 gap-12 items-center">
              <div>
                ...
                <div className="flex flex-wrap gap-3">
                  <button type="button" onClick={openSheet} className="inline-flex items-center gap-2 bg-brand-green hover:bg-brand-greenDark text-white rounded-full px-7 py-4 font-semibold transition-all hover:-translate-y-0.5 shadow-[0_10px_26px_rgba(24,168,74,0.32)]" data-testid="pos-lp-cta-primary">
                    Book a Free POS Demo <ArrowRight className="w-5 h-5" />
                  </button>
                </div>
                <div className="mt-4 flex flex-wrap items-center gap-2" data-testid="pos-lp-works-with">
                ...
                <p className="mt-2 text-xs text-brand-muted" data-testid="pos-lp-migration-note">Free data migration included · No lock-in · Cancel anytime</p>
              </div>
        ...
        <section className="bg-brand-sand py-20 sm:py-24" data-testid="pos-lp-proof">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <p className="mt-6 text-[10px] text-brand-muted" data-testid="pos-lp-proof-disclaimer">*Based on internal case studies &amp; partner results. Individual results may vary.</p>
          </div>
        </section>
      ...
      <DemoBottomSheet sector="restaurant-pos" open={sheetOpen} onClose={() => setSheetOpen(false)} />
    </div>
  );
}
