...
const FAQ_SCHEMA = {
  ...
  mainEntity: [
    { "@type": "Question",
      name: "Is MyGenie a good fit for a restaurant opening for the first time?",
      acceptedAnswer: { "@type": "Answer", text: "Yes. MyGenie works for restaurants at every stage — including outlets that have never used a POS before. Setup takes under 24 hours, no hardware purchase is needed (runs on any Android phone or tablet), and a dedicated onboarding specialist is included from day one. Starting at ₹799/outlet/month, there's no upfront cost and no long-term contract." } },
    { "@type": "Question",
    ...
    { "@type": "Question",
      name: "How does MyGenie pricing compare to other restaurant POS options?",
      acceptedAnswer: { "@type": "Answer", text: "MyGenie pricing starts at ₹799/outlet/month, published upfront — no calls, no quotes, no surprises. Most POS systems either require a ₹15,000–30,000 terminal purchase plus a monthly fee, or hide pricing behind a sales inquiry. MyGenie's Starter plan includes billing, inventory, KOT and daily reports. Growth (₹1,299/month) adds aggregator sync, CRM and Captain App. See the full breakdown on the pricing page." } },
    { "@type": "Question",
...
// ─── [CR-259] QuickDemoSheet extracted → DemoBottomSheet.jsx ─────────────────
// Stub retained so git blame is clear; all logic lives in DemoBottomSheet.jsx
// [removed — moved to DemoBottomSheet.jsx]
    ...
    <footer className="bg-brand-deep border-t border-[#1e4a2e]" data-testid="landing-footer">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex flex-col sm:flex-row items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center justify-center gap-4 text-xs text-[#5B7A68]">
          <span className="text-brand-muted" data-testid="landing-footer-phone">{COMPANY.phone}</span>
...
function VspHero({ doc }) {
  ...
  return (
    <section
      className="bg-brand-sand pt-20 pb-16 lg:pt-40 relative overflow-hidden"
...
export default function PetpoojaAlternative() {
  ...
  return (
    <div className="bg-white" data-testid="petpooja-alternative-page">
      ...
      <LandingFooter />
      <StickyMobileCta onDemo={() => setSheetOpen(true)} />
      <DemoBottomSheet sector="petpooja-alternative" open={sheetOpen} onClose={() => setSheetOpen(false)} />
    </div>
  );
}
