        ...
        <ul className="space-y-2" data-testid="menu-file-list">
          {files.map((f, i) => (
            <li key={i} className="flex items-center gap-3 text-sm bg-brand-sand/60 rounded-lg px-3 py-2">
              ...
              <button type="button" onClick={() => remove(i)} className="text-brand-muted hover:text-red-500"><X className="w-4 h-4" /></button>
            </li>
          ))}
        </ul>
      )}
...
export default function PaymentSuccess() {
  ...
  return (
    <div className="bg-white min-h-screen" data-testid="payment-success-page">
      <Seo title="Payment Successful | MyGenie POS" description="Your MyGenie POS plan is confirmed." path="/payment-success" noindex={true} />
      <Navbar />
