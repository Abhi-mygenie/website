...
import { TrendingUp, ArrowUpCircle, Plus, Check } from "lucide-react";
...
import FeatureDemoModal from "@/components/pricing/FeatureDemoModal";
...
import PlanCompareModal from "@/components/pricing/PlanCompareModal";
...
const PLAN_DISPLAY = PLANS.map(({ id, name, price, tagline, popular, includes, contactOnly }) => ({ id, name, price, tagline, popular, includes, contactOnly }));
...
export default function Pricing() {
  ...
  const [demoAddon, setDemoAddon] = useState(null);
  ...
  // Changing plan resets manual add-on picks; the new plan's included add-ons
  // auto-attach (derived). Avoids "phantom paid add-on after switching".
  const selectPlan = (id) => {
  ...
  const upsell = useMemo(() => {
    ...
    if (!next || next.contactOnly) return null;
  ...
  const monthly = (plan.price || 0) + onAddons.reduce((s, a) => s + a.price, 0);
  const total = monthly * MONTHS_PER_YEAR;
  const gst = Math.round(total * GST_RATE);
  const totalWithGst = total + gst;
  const isCustom = plan.contactOnly;
  const config = { plan, addons: onAddons, billing: "annual", monthly, total, gst, totalWithGst, outletType, wasRecommended };
  ...
  return (
    <div className="bg-white" data-testid="pricing-page">
      ...
      <FeatureDemoModal addon={demoAddon} open={!!demoAddon} onClose={() => setDemoAddon(null)} />
      <PlanCompareModal open={compareOpen} onClose={() => setCompareOpen(false)} />
      ...
      <main className="pt-28 pb-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="lg:col-span-2 space-y-10">
              ...
              <div id="plans">
                <div className="flex flex-wrap items-center justify-between gap-y-2 mb-5">
                  <h2 className="font-display text-xl sm:text-2xl font-bold text-brand-ink">1. Choose your base plan</h2>
                  <button
                    type="button"
                    onClick={() => setCompareOpen(true)}
                    data-testid="compare-plans-btn"
                    className="text-sm font-semibold text-brand-green hover:underline flex items-center gap-1 shrink-0"
                  >
                    Compare all plans ↓
                  </button>
                </div>

                {/* Scroll hint banner — appears above plan grid after first manual plan click */}
                <AnimatePresence>
                  {planClicked && !isCustom && (
                    <motion.div
                      initial={{ opacity: 0, y: -8 }}
                      animate={{ opacity: 1, y: 0 }}
                      exit={{ opacity: 0, y: -8 }}
                      transition={{ duration: 0.25 }}
                      className="mb-4 rounded-2xl border-2 border-brand-green bg-brand-green/[0.06] px-5 py-4 flex items-center gap-4"
                      data-testid="plan-selected-banner"
                    >
                      <span className="w-8 h-8 rounded-full bg-brand-green flex items-center justify-center shrink-0">
                        <Check className="w-4 h-4 text-white" strokeWidth={2.5} />
                      </span>
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-brand-ink text-sm">{plan.name} selected!</p>
                        <p className="text-xs text-brand-muted">Now pick the add-ons you need below</p>
                      </div>
                      <button
                        type="button"
                        onClick={() => document.getElementById("addons")?.scrollIntoView({ behavior: "smooth" })}
                        data-testid="see-addons-btn"
                        className="bg-brand-green text-white rounded-full px-4 py-2 text-sm font-semibold hover:bg-brand-greenDark transition-all shrink-0"
                      >
                        See add-ons ↓
                      </button>
                    </motion.div>
                ...
                <EditableList
                  ...
                  render={() => (
                    <div className="grid sm:grid-cols-2 gap-5">
                      {PLANS_M.map((p) => (
                        <PlanCard key={p.id} plan={p} selected={selectedPlanId === p.id} recommended={recPlanId === p.id} onSelect={selectPlan} onFeatureDemo={setDemoPlan} />
                      ))}
                    </div>
                  )}
                />
              ...
              {/* Upsell nudge — hidden for Custom plan */}
              {upsell && !isCustom && (
              ...
              {/* Add-ons — hidden for Custom (contact-only) plan */}
              {!isCustom && (
              <div id="addons">
                <div className="flex flex-wrap items-baseline justify-between gap-y-2 mb-5">
                  <h2 className="font-display text-xl sm:text-2xl font-bold text-brand-ink">2. Add modules you need</h2>
                ...
                <EditableList
                  ...
                  render={() => (
                    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                      {ADDONS_M.map((a) => <AddonCard key={a.id} addon={a} state={addonState(a.id)} onToggle={toggleAddon} onDemo={setDemoAddon} />)}
                    </div>
                  )}
                />
                ...
                {crossSell.length > 0 && (
                  <div className="mt-6 rounded-2xl bg-brand-green/[0.05] border border-brand-green/25 p-5" data-testid="cross-sell">
                    ...
                    <div className="mt-3 flex flex-wrap gap-3">
                      {crossSell.map((a) => (
                        <button type="button" key={a.id} onClick={() => toggleAddon(a.id)} data-testid={`cross-sell-${a.id}`} className="inline-flex items-center gap-2 rounded-full bg-white border border-brand-green/30 px-4 py-2 text-sm font-semibold text-brand-ink hover:bg-brand-green hover:text-white transition-all">
          ...
          {/* CR-142 — contextual link to /petpooja-alternative (internal PageRank) */}
          <div className="mt-8 text-center">
            <p className="text-sm text-brand-muted">
              Comparing with Petpooja?{" "}
              <Link
                to="/petpooja-alternative"
                data-testid="pricing-petpooja-link"
        ...
        <section
          id="lp-demo"
          className="bg-brand-deep rounded-3xl py-16 sm:py-20 px-8 sm:px-12 mt-16 scroll-mt-20"
