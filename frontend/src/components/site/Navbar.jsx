    ...
    <header data-testid="navbar" className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "bg-white/95 backdrop-blur-md shadow-[0_2px_20px_rgba(0,0,0,0.05)]" : "bg-white/80 backdrop-blur-sm"}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-[72px] flex items-center justify-between">
        ...
        <div className="hidden lg:flex items-center gap-3">
          ...
          {onDemo ? (
            <button type="button" onClick={() => onDemo()} data-testid="nav-demo-btn" className="bg-brand-green hover:bg-brand-greenDark text-white rounded-full px-6 py-2.5 font-semibold text-[15px] transition-all hover:-translate-y-0.5 shadow-[0_6px_18px_rgba(24,168,74,0.3)]">Book a Free Demo</button>
          ) : (
        ...
        <div className="lg:hidden flex items-center gap-2">
          {onDemo ? (
            <button type="button" onClick={() => onDemo()} data-testid="nav-mobile-demo-btn" className="bg-brand-green text-white rounded-full px-4 py-2 text-sm font-semibold shadow-[0_4px_12px_rgba(24,168,74,0.3)]">Book a Free Demo</button>
          ) : (
            <a href="/#demo" onClick={handleDemoCtaClick} data-testid="nav-mobile-demo-btn" className="bg-brand-green text-white rounded-full px-4 py-2 text-sm font-semibold shadow-[0_4px_12px_rgba(24,168,74,0.3)]">Book a Free Demo</a>
          )}
          <button type="button" className="p-2.5 text-brand-ink" onClick={() => setOpen(!open)} data-testid="nav-mobile-toggle" aria-label="Menu">
            {open ? <X /> : <Menu />}
          </button>
        </div>
      </div>
