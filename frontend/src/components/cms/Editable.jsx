...
function PencilButton({ id, onOpen }) {
  return (
    <span
      ...
      className="absolute top-2 right-2 z-30 w-6 h-6 rounded-full bg-emerald-600 text-white flex items-center justify-center shadow cursor-pointer opacity-0 group-hover/cms:opacity-100 transition"
    >
  ...
  return (
    <Modal title="Edit text" onClose={onClose}>
      ...
      <div className="flex justify-end gap-2">
        <button type="button" onClick={onClose} className="px-4 py-2 rounded-lg border border-brand-line">
          Cancel
        </button>
        <button
          type="button"
  ...
  return (
    <Modal title="Edit media" onClose={onClose}>
      ...
      <p className="text-xs text-gray-400 mb-3">
        Video spec: MP4 · H.264 · 1280×720 · max 20MB · ≤2Mbps. Use HandBrake (free) to compress before uploading.
      </p>
    ...
    <div className="fixed inset-0 z-[120] flex justify-end bg-black/40" onClick={onClose}>
      ...
      >
            ...
            <div className="flex justify-between mb-2">
              ...
              {!lockItems ? (
                <div className="flex gap-1">
                  <button type="button" onClick={() => move(i, -1)} className="text-xs px-2 py-1 border border-brand-line rounded">↑</button>
                  <button type="button" onClick={() => move(i, 1)} className="text-xs px-2 py-1 border border-brand-line rounded">↓</button>
                  <button type="button" onClick={() => remove(i)} className="text-xs px-2 py-1 border border-brand-line rounded text-red-600">Delete</button>
                </div>
              ) : null}
            </div>
            {fields.map((f) => {
        ...
        <div className="flex justify-end gap-2 sticky bottom-0 bg-white py-3">
          <button type="button" onClick={onClose} className="px-4 py-2 rounded-lg border border-brand-line">Cancel</button>
          <button
            type="button"
            onClick={save}
            disabled={busy}
    ...
    if (vidPlaying) {
      mediaEl = (
        <video
          ...
          data-testid="editable-video-player"
        />
      );
    } else {
      mediaEl = (
        <div className="relative cursor-pointer group/play" onClick={() => setVidPlaying(true)} data-testid="editable-video-play">
          <video src={src} preload="metadata" muted className={`${className} h-auto`} />
