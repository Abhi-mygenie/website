...
export default function RecommendQuiz({ onRecommend, reason }) {
  const [a, setA] = useState({ outletType: "", outlets: "", volume: "", priority: "" });
  const ready = a.outletType && a.outlets && a.volume && a.priority;
  ...
  return (
    <div className="rounded-3xl border-2 border-brand-green/25 bg-brand-green/[0.04] p-7 sm:p-8" data-testid="recommend-quiz">
      ...
      <div className="grid sm:grid-cols-2 gap-4 mt-5">
        ...
        <QuizSelect name="priority" label="Your #1 priority?" opts={QUIZ.priority} value={a.priority} onChange={set} />
      </div>
