...
def _base_row(doc, lead_type):
    ...
    return {
        ...
        "ad_set":  attr.get("utm_content") or None,
        "keyword": attr.get("utm_term") or None,
        "ad_name": attr.get("utm_ad") or None,
...
def _normalise_demo(doc):
    ...
    row.update({
        ...
        "crm_status":      doc.get("crm_status"),
        "crm_lost_reason": doc.get("crm_lost_reason"),
    })
    return row
...
async def query_leads(db, *, type=None, verified=None, paid=None, city=None,
    ...
    def keep(r):
        ...
        if paid is True and not r.get("paid"):
            return False
        if stage and r.get("crm_status") != stage:
            return False
        if cityl and cityl not in (r.get("city") or "").lower():
            return False
        if ql:
            blob = " ".join(str(r.get(k) or "") for k in
                            ("name", "phone", "email", "city", "campaign", "ad_set", "keyword", "ad_name", "summary")).lower()
            if ql not in blob:
                return False
        if df or dt_to:
    ...
    demo_scheduled_count = sum(1 for r in filtered if (r.get("crm_status") or "") in {"demo_scheduled","demo_given","won","lost"})
    demo_given_count     = sum(1 for r in filtered if (r.get("crm_status") or "") in {"demo_given","won","lost"})
    won_count            = sum(1 for r in filtered if r.get("crm_status") == "won")
    lost_count           = sum(1 for r in filtered if r.get("crm_status") == "lost")
    summary = {
        "total": len(filtered),
        "today": today_count,
