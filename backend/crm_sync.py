...
SOURCE_SYNC_AFTER_DATE = os.environ.get("SOURCE_SYNC_AFTER_DATE", "2026-06-20")
...
async def run_source_sync(db) -> dict:
    """Pull all Freshsales contacts where first_source is not empty, created after SOURCE_SYNC_AFTER_DATE.
    ...
    try:
        ...
        started = datetime.now(timezone.utc)
        logger.info("source_sync: starting — contacts with first_source after %s", SOURCE_SYNC_AFTER_DATE)
        ...
        while True:
            data = await _fs_search([
                ...
                {"attribute": "created_at", "operator": "is_after", "value": SOURCE_SYNC_AFTER_DATE},
        ...
        logger.info("source_sync: discovered %d contacts with first_source after %s", len(all_contacts), SOURCE_SYNC_AFTER_DATE)
