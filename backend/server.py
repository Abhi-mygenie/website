...
async def cms_config():
    ...
    """
    return {"leads_enabled": os.environ.get("LEADS_DASHBOARD_ENABLED", "true").lower() == "true"}
...
# ── CR-68: Churned Clients Dashboard ─────────────────────────────────────

@api_router.get("/cms/churn-report")
async def cms_churn_report(
    admin: str = Depends(cms_auth.get_dashboard_admin),
):
    churn_url   = os.environ.get("CHURN_API_URL", "")
    churn_token = os.environ.get("CHURN_API_TOKEN", "")

    if not churn_url or not churn_token:
        raise HTTPException(status_code=503, detail="Churn API not configured")

    try:
        async with httpx.AsyncClient(timeout=15) as client_h:
            resp = await client_h.get(
                churn_url,
                headers={"Accept": "application/json", "X-Churn-Token": churn_token},
            )
            resp.raise_for_status()
            payload = resp.json()
    except httpx.HTTPStatusError as e:
        raise HTTPException(status_code=502, detail=f"Churn API error: {e.response.status_code}")
    except Exception as e:
        raise HTTPException(status_code=502, detail=f"Churn API unreachable: {str(e)}")

    records = payload.get("data", [])

    counts = {"d3": 0, "d7": 0, "d30": 0, "d30plus": 0}
    bucketed = []
    for r in records:
        d = r.get("inactive_days", 0)
        if d >= 60:
            b = "d30plus"
        elif d >= 30:
            b = "d30"
            ...
            continue  # inactive 0-2 days = active clients, exclude from churn view
        counts[b] += 1
        rs = r.get("restaurant_status") or {}
        bucketed.append({
            "restaurant_id":        r.get("restaurant_id"),
            "restaurant_name":      r.get("restaurant_name", ""),
            "inactive_days":        d,
            "last_order":           r.get("last_order"),
            "orders_last_7_days":   r.get("orders_last_7_days", 0),
            "orders_last_30_days":  r.get("orders_last_30_days", 0),
            "last_30_days_revenue": r.get("last_30_days_revenue", 0),
            "active":               rs.get("active"),
            "status":               rs.get("status"),
            "bucket":               b,
        })

    bucketed.sort(key=lambda x: x["inactive_days"], reverse=True)
    ...
    return {
        ...
        "active_count":   sum(1 for r in records if (r.get("inactive_days") or 0) <= 2),
        "active_7_count": sum(1 for r in records if (r.get("inactive_days") or 0) <= 7),
        "at_risk_count":  sum(1 for r in records if (r.get("inactive_days") or 0) >= 7),
        "records": bucketed,
    }
...
async def cms_ads_mcp_status(admin: str = Depends(cms_auth.get_dashboard_admin)):
...
async def cms_ads_mcp_meta_refresh_token(admin: str = Depends(cms_auth.get_dashboard_admin)):
...
async def cms_leads_quality_summary(admin: str = Depends(cms_auth.get_dashboard_admin)):
    return await funnel_module.get_lead_quality_breakdown(db)


@api_router.post("/cms/ads/ai-insights")
async def cms_ads_ai_insights(admin: str = Depends(cms_auth.get_dashboard_admin)):
...
async def cms_ads_strategy_brainstorm(admin: str = Depends(cms_auth.get_dashboard_admin)):
...
async def google_ads_auth_url(request: Request, admin: str = Depends(cms_auth.get_dashboard_admin)):
...
async def google_ads_status(admin: str = Depends(cms_auth.get_dashboard_admin)):
...
async def cms_sync_status(admin: str = Depends(cms_auth.get_dashboard_admin)):
    return await funnel_module.get_sync_status(db)


@api_router.post("/cms/sync/trigger")
async def cms_sync_trigger(admin: str = Depends(cms_auth.get_dashboard_admin)):
...
async def cms_sync_backfill(admin: str = Depends(cms_auth.get_dashboard_admin)):
    import asyncio as _asyncio
...
async def cms_sync_source_backfill(admin: str = Depends(cms_auth.get_dashboard_admin)):
...
async def cms_sync_otp_backfill(admin: str = Depends(cms_auth.get_dashboard_admin)):
...
async def cms_sync_source_sync(admin: str = Depends(cms_auth.get_dashboard_admin)):
...
async def ad_spend_history(admin: str = Depends(cms_auth.get_dashboard_admin)):
...
async def ad_spend_delete(upload_id: str, admin: str = Depends(cms_auth.get_dashboard_admin)):
